...
백엔드 서버가 중지된 상태에서 인덱스 페이지에 접속하면 데이터 패칭 실패로 인해 Fetch failed 오류가 발생한다.
...
이런 방식은 데이터 패칭처럼 오류가 발생할 가능성이 있는 모든 코드 블록에 일일이 try-catch를 추가해야 하며, 예상하지 못한 오류에도 대응해야 하므로 관리가 어렵다.
